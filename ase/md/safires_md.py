...
class SAFIRES(MolecularDynamics):
    ...
    in a simulation: 1) a solute, which can be a particle / molecule
    ...
    SAFIRES assumes that the inner and outer solvent molecules 
    have the same mass. The position of the boundary is flexible with 
    respect to the inner region particle that is furthest away from 
    the solute. If a boundary event is detected, SAFIRES will redirect 
    the involved inner and outer region particles by performing an
    elastic collision between them, using the boundary as mediator.
    This propagator reduces to the Langevin propagator for a constant 
    time step and to the Velocity Verlet propagator for a constant time
    ...
    "outer"/"OUTER": referring to particles in the outer
                     region (atom.tag = 2).
    "ignore" : referring to atom or particle which SAFIRES
               ignores (atom.tag = 3). Special case for 
               counterions used in QM/MM simulations.
    "boundary": the boundary sphere that separates the inner 
                and outer region. Defined by the distance
    ...
    ------
    1) atoms = Atoms([...])
        atom.tag = 1 --> solute molecule or periodic surface
        atom.tag = 2 --> inner region
        atom.tag = 3 --> outer region
        atom.tag = 0 --> ignore

    2) MD = SAFIRES(atoms=atoms,
    ...
    def __init__(self, atoms, timestep, temperature=None, friction=None,
                 ...
                 natoms=None, natoms_in=None, safires_logfile=None,
                 debug=False, barometer=False, surface=False,
                 reflective=False):
        ...
        natoms -- number of atoms in solvent molecules
        natoms_in -- number of atoms in inner region solvent molecules
                     defaults to natoms if False
        ...
        self.surface --
            ...
            a surface. Surface must be perpendicular to
            z axis, no oblique or stepped surfaces possible.

        self.constraints --
            placeholder for atomic constraints if any 

        self.nout and self.nin --
            int number of atoms in each solvent molecule of the outer
            and inner region, respectively
            
        self.nall --
            array handling number of atoms in solute, inner and outer
            region molecules.

        self.idx_real --
            index for relative placement of pseudo-particles in 
            the atoms object
        ...
        # (i.e. shake/rattle, FixLinearTriatomic)
        
        ## setup output logfiles
        #if self.safires_logfile is not None:
        #    self.saf_log = open(self.safires_logfile, "w+")
        #    self.saf_log.write("iteration   boundary_idx   d_INNER / A\n")
    ...
    def safires(self, checkup=False):
        ...
        for atom in [tmpatm for tmpatm in com_atoms if tmpatm.tag == 3]:
