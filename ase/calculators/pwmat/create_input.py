...
class GeneratePWmatInput:
    ...
    def write_etot_input(self,
        ...
        # Type 2. int params
        self.int_params.update(
            {k.upper(): v for k, v in kwargs.items()
             if k.upper() in int_keys})
        ...
        # Type 3. bool params
        self.bool_params.update(
            {k.upper(): v for k, v in kwargs.items()
             if k.upper() in bool_keys})
        ...
        # Type 4. char params
        self.char_params.update(
            {k.upper(): v for k, v in kwargs.items()
             if k.upper() in char_keys})
        ...
        for tmp_idx, tmp_symbol in enumerate(atoms.get_chemical_symbols()):
            ...
            char_dct.update(
                {f"IN.PSP{tmp_idx+1}": f"{tmp_symbol}.SG15.PBE.UPF"})
        etot_input_params.update(char_dct)
        # Type 5. special params
        self.special_params.update(
            {k.upper(): v for k, v in kwargs.items()
             if k.upper() in special_keys})
        ...
        self.string_params.update(
            {k.upper(): v for k, v in kwargs.items()
             if k.upper() in string_keys})
        ...
        self.list_int_params.update(
            {k.upper(): v for k, v in kwargs.items()
             if k.upper() in list_int_keys})
