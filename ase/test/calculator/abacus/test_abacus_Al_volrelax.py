...
def test_abacus_Al_volrelax(factory):
    """
    Run ABACUS tests to ensure that relaxation with the VASP calculator works.
    ...
    Al_vasp = abacus_vol_relax()
    Al_ase = ase_vol_relax()

    assert cells_almost_equal(Al_ase.get_cell(), Al_vasp.get_cell())
